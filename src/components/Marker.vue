  ...
  <mgl-marker :coordinates="props.coordinates">
    <template v-slot:marker>
      <div class="marker-box cursor-pointer bottom-0 -translate-x-1/2" @click="$emit('markerClick')">
        ...
        <div class="marker-status pl-1 pr-1 pt-0.5 pb-0.5"
          :class="{ 'marker-s1': props.status === 1, 'marker-s2': props.status === 2, 'marker-s3': props.status === 3,
           'marker-s4': props.status === 4, 'marker-s5': props.status === 5, 'marker-s6': props.status === 6,
            'marker-s7': props.status === 7, 'marker-s8': props.status === 8, 'marker-s9': props.status === 9,
