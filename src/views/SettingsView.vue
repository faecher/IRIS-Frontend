...
function loadMCPConfig() {
  // Try loading data
  axios.get(base.root_url + '/api/mcp/config', {
    headers: {
...
function loadMCPOperations() {
  // Try loading data
  axios.get(base.root_url + '/api/mcp/operations', {
    headers: {
      'Content-Type': 'application/json'
    }
  }).then(function (response) {
...
function selectOperation(item: Operation) {
  axios.post(base.root_url + '/api/mcp/operations/enable', {
    uid: item.uid
  }, {
    headers: {
      'Content-Type': 'application/json'
...
function deselectOperation(item: Operation) {
  axios.post(base.root_url + '/api/mcp/operations/disable', {
    uid: item.uid
  }, {
    headers: {
      'Content-Type': 'application/json'
...
function handleMCPSubmit() {
  axios.post(base.root_url + '/api/mcp/start', {
    url: url.value,
...
function resetTrackers() {
  alert("Das ist noch nicht implementiert!")
}
...
<template>
  <div class="p-5 min-w-64">
    <RouterLink to="/">
      <p class="text-sm text-blue-800">Zurück zur Karte</p>
    </RouterLink>
    <h1 class="font-bold text-5xl mt-2 mb-6">Einstellungen</h1>
    <div v-if="hasError"
    ...
    <h2 class="font-bold text-2xl mb-2">MCP-Verbindung</h2>
    <p class="mb-2 text-sm font-medium text-gray-900">MCP-Integration:
    ...
    <form @submit.prevent="handleMCPSubmit">
      <StringInput id="url" label="Server-URL" v-model="url" placeholder="https://127.0.0.1:443"/>
      <StringInput id="api_key" label="API-Key" v-model="api_key"/>
      <button type="submit"
              class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 focus:outline-none">
        Mit MCP verbinden
      </button>
    </form>
    <h2 class="font-bold text-2xl mb-2">MCP-Sanitätsdienst</h2>
    <div v-if="operations.length === 0 || !enabled">
      <p class="text-gray-500">Keine aktiven Sanitätsdienste gefunden!</p>
    </div>
    <div v-else>
      <p class="text-gray-900">Alle aktiven Sanitätsdienste</p>
      <div v-for="operation in operations" class="flex items-center p-1 hover:bg-gray-100">
        <p class="font-semibold text-lg">{{ operation.title }}</p>
        <p v-if="operation.selected" class="ml-2 text-lg">(Ausgewählt)</p>
        <div class="grow"></div>
        <button
    ...
    <h2 class="font-bold text-2xl mb-2">Tracker zurücksetzen</h2>
    <button
        @click="resetTrackers"
        class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 focus:outline-none">
