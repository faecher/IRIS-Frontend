...
import {Event, Evented} from 'maplibre-gl';
import type {Map} from 'maplibre-gl';
import {type Popup, type Offset} from 'maplibre-gl';
...
export class Marker extends Evented {
    ...
    constructor(options?: MarkerOptions) {
        ...
        if (!options || !options.element) {
            ...
            label.classList.add('marker-text');
            label.classList.add('ml-1');
            label.classList.add('mr-1');
            label.classList.add('p-0.5');

            this._element.appendChild(label);

            statusIndicator.textContent = this._status?.toString() ?? "?";
            statusIndicator.classList.add('marker-status');
            statusIndicator.classList.add('pl-1');
            statusIndicator.classList.add('pr-1');
            statusIndicator.classList.add('pt-0.5');
            statusIndicator.classList.add('pb-0.5');

            if (this._status === undefined) {
                statusIndicator.classList.add("marker-unknown")
            } else {
                statusIndicator.classList.add('marker-s' + this._status.toString())
            }

            this._element.appendChild(statusIndicator);
